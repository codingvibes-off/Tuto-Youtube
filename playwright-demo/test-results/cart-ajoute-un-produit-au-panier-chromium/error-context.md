...
- Name: cart.spec.ts >> ajoute un produit au panier
- Location: tests/cart.spec.ts:3:5
...
```ts
  ...
  2  | 
  3  | test('ajoute un produit au panier', async ({ page }) => {
  4  |   await page.goto('/');
  5  | 
...
> 10 |   await page.getByRole('button', { name: 'Add to cart' }).click();
     |                                                           ^ Error: locator.click: Error: strict mode violation: getByRole('button', { name: 'Add to cart' }) resolved to 6 elements:
  11 | 
  12 |   // le badge du panier doit afficher "1"
  13 |   await expect(page.locator('.shopping_cart_badge')).toHaveText('1');
  14 | });
  15 | 
```